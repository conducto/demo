...
* Lazily define your pipeline at runtime with `co.lazy_py`.
...
def run() -> co.Serial:
    ...
    with co.Serial(image=IMG, doc=co.util.magic_doc()) as output:
        ...
        output["parallel_word_count"] = n = co.lazy_py(
...
def parallelize(wordlist_path, result_dir, top: int, chunksize: int) -> co.Parallel:
    ...
    pipeline, and call it with `co.lazy_py`.
    ...
    ```python
    output["parallel_word_count"] = co.lazy_py(
        parallelize, WORDLIST_PATH, RESULT_DIR, top=15, chunksize=1000
    )
    ```
    ...
    #### Requirements
    `co.lazy_py` has all the same limitations as `co.Exec(func)` as you saw in
    ...
    #### When to use it
    This pipeline uses `co.lazy_py` to dynamically parallelize over some input
